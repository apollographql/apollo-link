...
describe('Link utilities:', () => {
  ...
  describe('makePromise', () => {
    ...
    it('return next call as Promise resolution', () => {
      return LinkUtils.makePromise(Observable.of(data))
        .then(result => assert.deepEqual(data, result));
    });

    it('return error call as Promise rejection', () => {
      return LinkUtils.makePromise(new Observable(observer => observer.error(error)))
        .then(expect.fail)
    ...
    describe('warnings', () => {
      ...
      it('return error call as Promise rejection', (done) => {
        spy.callsFake(() => done());
        LinkUtils.makePromise(Observable.of(data, data))
          .then(result => assert.deepEqual(data, result));
      });
    });
  });
