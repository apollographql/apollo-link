...
You can also use Apollo Link with `graphql-tools` to facilitate schema stitching by using `cross-fetch` as your request link's fetcher function and passing it to `makeRemoteExecutableSchema`.
